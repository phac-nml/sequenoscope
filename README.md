...
- **Test Directory:** Provide the path to the test directory that contains the __sequence manifest__ files from the analyze module. `-T` or `--test_dir` `<test_dir_path>`
- **Control Directory:** Specify the path to the control directory that contains the __sequence manifest__ files from the analyze module. `-C` or `--control_dir` `<control_dir_path>`
