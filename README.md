...
- [Outputs](#outputs)
  ...
  - [Mapping of terminal `end_reason` values to Adaptive Sampling read decision categories](#mapping-of-terminal-end_reason-values-to-adaptive-sampling-read-decision-categories)
- [Citation](#citation)
- [Legal](#legal)
- [Contacts](#contacts)
...
👉 [nf-sequenoscope](https://github.com/phac-nml/nf-sequenoscope)

This approach ensures better scalability and reproducibility, especially in HPC or cloud environments.

## Workflow Example

In this section, we will walk through a simple workflow using mock data stored in [`mock_data`](./mock_data/) folder to demonstrate how to use each module of **sequenoscope**. The mock data directory contains the following files:

```
./mock_data/
├── mock_adaptive_sampling.fastq
├── mock_adaptive_sampling_summary.txt
├── mock_control.fastq
├── mock.fastq
...
>[!note]
> - [`mock_adaptive_sampling.fastq`](./mock_data/mock_adaptive_sampling.fastq):
> FASTQ file containing subsampled 1016 reads generated during an ONT adaptive sampling (AS) run (**test sample**). 
>- [`mock_adaptive_sampling_summary.txt`](./mock_data/mock_adaptive_sampling_summary.txt):
Adaptive sampling decision log produced by MinKNOW. This file records only reads that triggered an explicit ReadUntil decision (e.g., `unblock`, `no_decision`, `stop_receiving`) This log contains data exclusively for the **test sample**. Reads from the **control sample** are excluded by design as they do not undergo adaptive filtering.
>- [`mock_control.fastq`](./mock_data/mock_control.fastq): FASTQ file from a non-adaptive (**control sample**) ONT sequencing run, used as a baseline for comparison against adaptive sampling results and contains 1017 reads.
>- [`mock.fastq`](./mock_data/mock.fastq): An overall FASTQ example used for basic pipeline testing and demonstration purposes containing 2023 reads corresponding both to the **test** and **control** samples.
>- [`mock_reference.fasta`](./mock_data/mock_reference.fasta): Reference genome FASTA file with total of 8 reference complete genomes used by the **analyze** module for read mapping and taxonomic assignment.
>- [`mock_sequencing_summary.txt`](./mock_data/mock_sequencing_summary.txt):
this **sequencing summary file** generated by MinKNOW during sequencing or produced post-run by Guppy or Dorado basecaller from POD5 files. This file contains per-read metadata (e.g., `read length`, `channel`, `barcode`, and importantly the `end_reason` fields) for **all reads** generated during the sequencing run, including both control and test samples, and serves as the primary source for inferring per-read adaptive sampling outcomes in Sequenoscope.
...
### Mapping of terminal `end_reason` values to Adaptive Sampling read decision categories
Sequenoscope infers per-read adaptive sampling outcomes using the `end_reason` field recorded in the **sequencing summary file** generated by MinKNOW, Guppy or Dorado basecallers (such as [`mock_sequencing_summary.txt`](./mock_data/mock_sequencing_summary.txt)). Because true ReadUntil API read decision logs are not always generated or available, these are post AS run approximations rather than direct ReadUntil decisions, which are usually recorded in the **adaptive sampling decision file** (an example [`mock_adaptive_sampling_summary.txt`](./mock_data/mock_adaptive_sampling_summary.txt) file is provided in the `mock_data` folder and does not include all AS test reads of [`mock_adaptive_sampling.fastq`](./mock_data/mock_adaptive_sampling.fastq)). Notably, this adaptive sampling summary file (e.g.,`mock_adaptive_sampling_summary.txt`) does not include all reads present in `mock_adaptive_sampling.fastq` file with 103 of 1,016 reads missing (~10%).

It is normal that adaptive sampling summary file does not contain all sequenced reads, as MinKNOW logs only reads that trigger an explicit ReadUntil decision and belong to the **test sample** undergoing adaptive sampling. Consequently, discrepancies are expected when comparing the adaptive sampling file to the raw FASTQ data or the standard **sequencing summary file**, both of which are comprehensive and include every read generated during the run.
