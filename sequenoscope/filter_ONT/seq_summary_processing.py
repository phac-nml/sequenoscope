...
class SeqSummaryProcesser:
    """
    Processes Oxford Nanopore (ONT) sequencing summary files to categorize and filter reads 
    based on adaptive sampling decisions and technical metadata.

    Because direct ReadUntil API decision logs are not always available,
    Sequenoscope infers per-read adaptive sampling outcomes using the
    terminal `end_reason` field reported in sequencing summary files
    generated by MinKNOW and Guppy or Dorado basecallers. These categories
    represent approximations of adaptive sampling behavior after the run
    has completed and do not necessarily reflect explicit real-time
    ReadUntil API decisions.

    This class maps ONT sequence summary 'end_reason' codes to simplified classifications:
    - 'stop_receiving': These are accepted reads where a target was identified and sequencing continued.
    - 'unblocked': These reads explicitly rejected by the ReadUntil API to free the pore.
    - 'no_decision':  These are reads for which no definitive adaptive sampling decision can be inferred
    (e.g., `signal_negative`, `unblock_mux_change`). This is a technical
    classification and does not indicate a biological “no match” due to supplied
    reference sequences failure; rather, it reflects reads that could not be
    confidently assigned to an adaptive sampling outcome due to technical reasons.
    - 'all': Includes all the above categories combined.
    """
    ...
    classes = {"stop_receiving":["signal_positive"], "unblocked":["data_service_unblock_mux_change"],
               ...
               "all":["signal_positive", "data_service_unblock_mux_change", "signal_negative", "unblock_mux_change"]}
